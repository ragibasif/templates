...
#define error_str(error, string) strerror_s(&string[0], sizeof(string), -error)
#else
#define error_str(error, string) strerror_r(-error, &string[0], sizeof(string))
#endif
...
static inline int dbg_format_backtrace_addr2line(void **addresses_pp,
    ...
    for (i = (depth - 1); i >= 0; i--) {
        snprintf(&command[0], sizeof(command), "addr2line -f -p -e %s %p",
                 &exe[0], ((void *)(((uintptr_t)(addresses_pp[i])) - 1)));
